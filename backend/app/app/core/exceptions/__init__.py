...
class JsonError(TLSReportingExceptionBase):
    ...
    def __init__(self, original_exception: Exception):
        super(JsonError, self).__init__(
            ...
            additional=original_exception.errors() if isinstance(original_exception, ValidationError) else None
        )
...
def openapi_examples(error_codes: frozenset = None) -> dict:
    ...
    for subclass in TLSReportingExceptionBase.__subclasses__():
        if error_codes is None or subclass.ERROR_CODE in error_codes:
            all_exceptions[subclass.__name__] = {
                'value': {
                    'detail': {
                        'code': subclass.ERROR_CODE,
                        'message': subclass.MESSAGE
                    }
                }
            }
    ...
    if len(all_exceptions) == 0:
        ...
        raise RuntimeError('Not all exceptions in "error_codes" could be matched with an error!')
